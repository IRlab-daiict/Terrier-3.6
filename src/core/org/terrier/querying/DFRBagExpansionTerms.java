...
public class DFRBagExpansionTerms extends ExpansionTerms {
	...
	* expanded terms.
	...
	public void insertDocument(FeedbackDocument doc, List terms) throws IOException
	{
		insertDocument(doc.docid, doc.rank, doc.score, terms);
	}
	
	public void insertDocument(int docid, int rank, double score, List terms) throws IOException
	{
		int doclen = 0;
		for (String term : terms.getItems()) {
			final IterablePosting ip = directIndex.getPostings((BitIndexPointer)documentIndex.getDocumentEntry(docid));
			if (ip == null)
			{
				logger.warn("document id "+docid+" not found");
			...
			while(ip.next() != IterablePosting.EOL)
			{
				if( lexicon.getLexiconEntry(ip.getId()).getKey().equals(term) )
				{
					this.insertTerm(ip.getId(), ip.getFrequency());
					//System.out.println("Term "+ term +" inserted");
					doclen++;
					//break;
				}
			}
