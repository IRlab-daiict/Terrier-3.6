			...
			//	System.err.println("TEXT NODE: " + parentNodeName.toLowerCase());
				...
				if(! DocIdIsAttribute && parentNodeName.toLowerCase().equals(DocIdLocation))
					...
					for(int i=0; i<attributes.getLength(); i++) {
						...
						if(n1.getNodeName().equals("pub-id-type") && n1.getNodeValue().equals("pmc"))
						{
...
//						}
					...
					//System.out.println(DocIdIsAttribute);
					//System.out.println(n.getNodeName()+" "+n.getTextContent()+" "+parent.hasAttributes());
	...
	*/
	...
	protected boolean openNextFile()
	{
		...
		if(logger.isInfoEnabled()){
			logger.info("Found "+Documents.size() + " documents ishita in "+filename);
		}
		xmlDoc = null;
		return true;
	}
